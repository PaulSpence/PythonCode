...
min_year=201
max_year=205
base_dir='/short/v45/pas561/mom/archive/gfdl_nyf_1080_PI_Ant_20Eto140E/10yr_5day/'
...
for n in range(min_year, max_year+1):
	for d in range(3,364,5):
		...
		var_avg[cnt-1] = sum(var*area*dzt)/sum(area*dzt)
...
output_file = base_dir + str(varname) +'_3Davg_' + str(min_year) +'to'+ str(max_year) + '_' + str(min_lon) + 'to' + str(max_lon) + '_' + str(min_lat) + 'to' + str(max_lat) + '_' + str(min_depth) + 'to' + str(max_depth) + '.nc'
